...
foreach($data->entity_relationships as $relationship){
	...
	try{
		if($relationship->relationship == 'member'){
			...
			$group = get_entity( $GUID->migrate($relationship->guid_two),'group');
			if(!($user instanceof ElggUser) || !($group instanceof ElggUser) && (!$user || !$group)){
				continue;
			}
			$group->join($user);
		echo "{$user->name} is now a member of {$group->name}\n";
		}
	} catch(Exception $e){
		$errors[] = $e->getMessage();
	}
