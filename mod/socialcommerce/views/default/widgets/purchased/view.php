...
<?php
	...
	if($display == 1){
		...
		$product = elgg_get_entities($options);
		//$product = get_entities("object", "order_item", elgg_get_page_owner_guid(),'', $number);
	}else if($display == 2){
		if ($friends = get_user_friends($user_guid, '', 999999, 0)) {
			...
			$product = elgg_get_entities($options);
			//$product = get_entities('object','order_item','',$friendguids,$number);
		}
	}else if($display == 3){
		//Depricated function replace
		$options = array('types'			=>	"object",
		...
		$product = elgg_get_entities($options);
		//$product = get_entities("object", "order_item", '', 0, $number);
	}
	...
	if ($product) {
		...
		foreach ($product as $f){
			$product_guid = $f->product_id;
			if(!isset($products[$product_guid])){
				$products[$product_guid] = $f; 
			}
		}
						...
						$cart_wishlist = <<<EOF
							<div style='display:block;'>
								<div class="cart_wishlist" style='float:left'>
									<form name="frm_wishlist2_{$f->guid}" method="POST" action="{$CONFIG->wwwroot}action/{$CONFIG->pluginname}/add_wishlist">
										<a title="{$wishlist_text}" class="wishlist" onclick=" document.frm_wishlist2_{$f->guid}.submit();" href="javascript:void(0);">&nbsp;</a>
