...
EOF;
	...
	if($product->status == 1){
		//$tell_a_friend = elgg_view("{$CONFIG->pluginname}/tell_a_friend",array('entity'=>$product,'text'=>"display"));
		$share_this = elgg_view("{$CONFIG->pluginname}/share_this",array('entity'=>$product));
		if($product->owner_guid != $_SESSION['user']->guid && $product_type_details->addto_cart == 1){
			$wishlist_action = $CONFIG->wwwroot."action/{$CONFIG->pluginname}/add_wishlist?pgid=".$product->guid."&__elgg_token=".generate_action_token($ts)."&__elgg_ts={$ts}";
...
EOF;
	...
	$version = get_latest_version($product_guid);
	if($version){
