<?php
	...
	if (!elgg_instanceof($category, 'object', 'category') || !$category->canEdit()) {
		register_error(elgg_echo('category:unknown'));
		forward(REFERRER);
	}
	
	// Render the category upload page
	$title = $title = elgg_echo('category:edit');
    ...
    $area2 = elgg_view("{$CONFIG->pluginname}/forms/edit_category",array('entity' => $category));
