			...
			<div>
				<?php $submit = get_input('btn_submit');
					if(!empty($submit)){
						...
						$shipping_rate = get_input('shipping_rate');
						$address_type = get_input('address_type');
						
						$weight = get_input('weight');
						
						$to_country = get_input('country');
						$to_state = get_input('state');
						$to_zip = get_input('postal_code');
						$destination = array('to_country'=>$to_country,'to_state'=>$to_state,'to_zip'=>$to_zip);
						
						$weight = array('weight'=>$weight,'to_unit'=>'lbs');
						
						$price = get_quote_ups($service_types,$packaging_type,$shipping_rate,$address_type,$destination,$origin,$weight);
				?>
					<div style="margin:0 auto;width:90%;font-size:13px;padding:5px;border:1px solid #cccccc;">
						<?php 
							if(!empty($price)){
								
								$display .= "<div>{$price}</div>";
							}
							echo $display;
						?>
					</div>
					...
					<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
						<table class="stores_settings" style="margin:0 auto;;" width="400">
							...
							<?php
								...
								$selected_country = "US";
								if($CONFIG->country){
									$country_list = '<select onkeyup="find_state(\''.$type.'\')"  onkeydown="find_state(\''.$type.'\')" onchange="find_state(\''.$type.'\')" name="country" id="'.$type.'_country" class="fedex_fields">';
									foreach ($CONFIG->country as $country){
										if($selected_country == $country['iso2']){
											$selected = "selected";
										}else{
											$selected = "";
										}
										$country_list .= "<option value='".$country['iso2']."' ".$selected.">".$country['name']."</option>";
									}
									$country_list .= "</select>";
									if($selected_country){
										$states = get_state_by_fields('iso2',$selected_country);
										if(!empty($states)){
