<?php
	...
	function set_checkout_settings_paypal_ipn(){
		...
		if(empty($error_field)){
			...
			$checkout_settings->checkout_method = 'paypal_ipn';
	...
	function checkout_payment_settings_paypal_ipn(){
		global $CONFIG;
		...
		/*echo $CONFIG->wwwroot."action/{$CONFIG->pluginname}/manage_socialcommerce?page_owner=".elgg_get_page_owner_guid().'&manage_action=makepayment&payment_method='.$method;
		echo "<br>{$method},".elgg_get_page_owner_guid().','.$BillingDetails->guid.','.$ShippingDetails->guid.','.$_SESSION['CHECKOUT']['shipping_method'];
		exit;*/
		$ts = time();
		$hiddenFields = array(
			'cmd'			=> '_xclick',
			...
			//'item_name'		=> $_SESSION['CHECKOUT']['item_name'],
			...
			//'notify_url'	=> $CONFIG->wwwroot."action/{$CONFIG->pluginname}/manage_socialcommerce?page_owner=".elgg_get_page_owner_guid().'&manage_action=makepayment&payment_method='.$method."&__elgg_token=".generate_action_token($ts)."&__elgg_ts={$ts}",
			'notify_url'	=> $CONFIG->wwwroot."{$CONFIG->pluginname}/manage_socialcommerce/".elgg_get_page_owner_guid()."/makepayment/".$method,
			// Customer details
	...
	function makepayment_paypal_ipn(){
		global $CONFIG;
		$custom = get_input('custom');
		//$custom = "paypal_ipn,35,66,,,258";
		if(!empty($custom)){
			...
			$cart_guid = $custome[5];
			//Depricated function replace
			$options = array(	'metadata_name_value_pairs'	=>	array('checkout_method' => 'paypal_ipn'),
							'types'				=>	"object",
							'subtypes'			=>	"s_checkout",
							'limit'				=>	1,
						);
			$settings = elgg_get_entities_from_metadata($options);
			//$settings = get_entities_from_metadata('checkout_method','paypal_ipn','object','s_checkout',0,1);
			if($settings){
				$settings = $settings[0];	
			}
			...
			$txn_id 			= '165s5sdf5s6d5sdfasdfsfasd5';
			$receiver_email 	= 'akhi.pv@gmail.com';
			$payer_email 		= $_POST['payer_email'];
			$new_fund 			= $payment_gross - $payment_fee;*/
			...
			if($process_payment){
				...
				$transactions = array('status'=>$ransaction_status,
									  ...
									  'fee'=>$payment_fee,
									  'total'=>$payment_gross);
				create_order($page_owner,$CheckoutMethod,$transactions,$BillingDetails,$ShippingDetails,$ShippingMethods,$cart_guid);	
			}
		}
		exit;
	}
	
	function varyfy_checkout_settings_paypal_ipn(){
		//Depricated function replace
		$options = array(	'metadata_name_value_pairs'	=>	array('checkout_method' => 'paypal_ipn'),
						'types'				=>	"object",
						'subtypes'			=>	"s_checkout",
		...
		if($settings){
			...
			if($missing_field != ""){
				return sprintf(elgg_echo('paypal_ipn:missing:fields'),$missing_field);
			}
			return;
		}else{
			return elgg_echo('not:fill:paypal_ipn:settings');
		}
	}
?>