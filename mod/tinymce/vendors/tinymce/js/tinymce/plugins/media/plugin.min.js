...
tinymce.PluginManager.add('media', function(editor, url) {
	var urlPatterns = [
		{regex: /youtu\.be\/([\w\-.]+)/, type: 'iframe', w: 425, h: 350, url: '//www.youtube.com/embed/$1', img: '//img.youtube.com/vi/$1/hqdefault.jpg'},
		{regex: /youtube\.com(.+)v=([^&]+)/, type: 'iframe', w: 425, h: 350, url: '//www.youtube.com/embed/$2', img: '//img.youtube.com/vi/$2/hqdefault.jpg'},
	...
	function dataToHtml(data) {
		...
		tinymce.each(urlPatterns, function(pattern) {
			...
			if ((match = pattern.regex.exec(data.source1))) {
				...
				for (i = 0; match[i]; i++) {
					...
					img = pattern.img.replace('$' + i, function() {
						return match[i];
					});
				}
		...
		if (data.embed) {
			...
			if (data.type == "iframe") {
				...
				html += '<img src="'+data.thumbnail+'" width="0" height="0"/>';
