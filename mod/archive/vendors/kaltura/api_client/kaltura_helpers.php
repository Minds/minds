...
class KalturaModel {
	...
	function getAdminSession($privileges = "")
    {
        ...
        $secret = elgg_get_plugin_setting('admin_secret', 'archive');
	...
	function addUploadToken($uploadToken)
	{
		if (!$this->session)
			$this->startSession();
...
class KalturaHelpers
{
	...
	function getContributionWizardFlashVars($ks, $entryId = null)
	{
		...
		$flashVars["partnerId"] 	= elgg_get_plugin_setting("partner_id","archive");
		$flashVars["subPartnerId"] 	= elgg_get_plugin_setting("subp_id", "archive");
		/*if ($entryId)
		    $flashVars["kshowId"] 		= "entry-".$entryId;
		else
		    $flashVars["kshowId"] 		= "-2";*/
		//$flashVars["afterAddentry"] = "renameAndEditMixVideo";
		//if(in_array(elgg_get_plugin_setting("alloweditor","archive"), array('simple','no'))) {
	...
	function getSimpleEditorFlashVars($ks, $entryId)
	{
		...
		$flashVars["partnerId"] 	= elgg_get_plugin_setting("partner_id","archive");
		$flashVars["subPartnerId"] 	= elgg_get_plugin_setting("subp_id", "archive");
		$flashVars["subpId"]     	= elgg_get_plugin_setting("subp_id", "archive");
	...
	function getPlayerByType($type)
	{
		...
		{
			//$player = $players[elgg_get_plugin_setting("kaltura_default_player_type","archive")];
			$player = $players['grey'];
		}
	...
	function getSwfUrlForBaseWidget($uiConf)
	{
		...
		return KalturaHelpers::getServerUrl() . "/index.php/kwidget/wid/_" . elgg_get_plugin_setting("partner_id","archive") . "/ui_conf_id/" . $uiConf;
	}

	function getSwfUrlForWidget($widgetId = null, $uiConfId = null)
	{
	    if (!$widgetId)
	        $widgetId = "_" . elgg_get_plugin_setting("partner_id","archive");
	...
	{
		$url = elgg_get_plugin_setting('kaltura_server_url', 'archive');
