...
function onEditorSave (entries) {
	if(entries != undefined) {
		...
		//hide the contribution wizard
		$('#kaltura_contribution_wizard').parent().html("<h2>Please wait...</h2><p>Your video is being saved</p>");
		$('#kaltura_contribution_wizard').remove();
		...
		// we need a synchronous call here
        $.ajax({	
			url:'<?php echo elgg_get_site_url(); ?>archive/kaltura/ajax-update/'+uploaded_entry_id, 
			success: function(data, response){ window.location = '<?php echo $refresh_loc; ?>' + uploaded_entry_id; },
