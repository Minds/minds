...
function polls_init() {
	...
	elgg_register_page_handler('vote','vote_page_handler');

	// Register a URL handler for poll posts
	elgg_register_entity_url_handler('object','poll','voting_url');
...
/**
 * voting page handler; allows the use of fancy URLs
 *
...
/**
 * vote page handler; allows the use of fancy URLs
 *
 * @param array $page From the page_handler function
 * @return true|false Depending on success
 */
function vote_page_handler($page) {
	elgg_load_library('elgg:polls');
	$page_type = $page[0];
	switch($page_type) {
		case "view":		
 ...
 */
function voting_url($poll) {
	$title = elgg_get_friendly_title($poll->title);
	return  "vote/view/" . $poll->guid . "/" . $title;
}
