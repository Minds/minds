...
try {
	...
	$amount = minds\plugin\bitcoin\bitcoin()->toBTC($order->amount_in_satoshi);
    ...
    $wallet = minds\plugin\bitcoin\bitcoin()->getWallet(elgg_get_logged_in_user_entity());
    if (!$wallet) throw new Exception('Could not retrieve your wallet');
    
    // Send payment
    if ($transaction_hash = minds\plugin\bitcoin\bitcoin()->sendPayment($wallet->guid, $address, minds\plugin\bitcoin\bitcoin()->toSatoshi($amount))) {
