...
if (preg_match("/^[_.a-zA-Z0-9]+$/", $jsonInput->service) === false)
{
...
//    if (ereg("^(https?://[^/]*)", $_SERVER["HTTP_REFERER"], $regs) === false)
    if (preg_match("|^(http.*?://[^/]*)|", $_SERVER["HTTP_REFERER"], $regs) === false)
    {
    ...
    if ($refererDomain != $requestUriDomain)
    {
        /* Nope. */
        $error->SetError(JsonRpcError_PermissionDenied,
                         "Permission Denied [3] $refererDomain != $requestUriDomain
Please make sure your domainname is set identical to domainname in your url.");
        $error->SendAndExit();
        /* never gets here */
    }
...
if (is_object($output) && get_class($output) == "JsonRpcError")
{
