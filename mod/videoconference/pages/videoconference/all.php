...
$sql = "SELECT ".$CONFIG->dbprefix."objects_entity.guid, ".
       ...
       "ON (".$CONFIG->dbprefix."objects_entity.guid = ".$CONFIG->dbprefix."entities.guid) ".
			 "WHERE ".$CONFIG->dbprefix."entity_subtypes.subtype = 'videoconference'; ";
...
if ($rooms = get_data($sql)) { 
	foreach($rooms as $room) {
		...
		$cleanup = $nilai[25];
		$timelastaccess = $nilai[33]; 
		$mastercleanup = (datalist_get('vconf_cleanUp') * 86400);
		...
		if (($cleanup>0) AND (($ztime-$timelastaccess)>$cleanup)) {
			...
			delete_data("delete from {$CONFIG->dbprefix}objects_entity where guid = {$room->guid} OR title = '".$room->title."'");
...
// end cleanup
    ...
    $ver=explode('.', get_version(true));
  	if ($ver[1]>7) {
      elgg_pop_breadcrumb();
      elgg_push_breadcrumb(elgg_echo('videoconference'));
    }

		$title = elgg_view_title(elgg_echo("videoconference:rooms"));
    ...
    if (!$options) {
    	$options = elgg_echo('videoconference:none');
    }
	...
	// Display page
  	if ($ver[1]>7) echo elgg_view_page(elgg_echo('videoconference:rooms'),$body, 'default', array( 'sidebar' => "" ));
    else page_draw(elgg_echo('videoconference:rooms'),$body);
