...
$sql = "SELECT ".$CONFIG->dbprefix."objects_entity.guid, ".
       ...
       "ON (".$CONFIG->dbprefix."objects_entity.guid = ".$CONFIG->dbprefix."entities.guid) ".
			 "WHERE ".$CONFIG->dbprefix."entity_subtypes.subtype = 'videoconsultation'; ";
...
if ($rooms = get_data($sql)) { 
	foreach($rooms as $room) {
		...
		$cleanup = $nilai[46];			
		$timelastaccess = $nilai[49];
		$mastercleanup = (datalist_get('vcons_availability') * 86400);
...
// end cleanup
    ...
    $ver=explode('.', get_version(true));
  	if ($ver[1]>7) {
      elgg_pop_breadcrumb();
      elgg_push_breadcrumb(elgg_echo('videoconsultation'));
    }

		$title = elgg_view_title(elgg_echo("videoconsultation:rooms"));
		
  	if ($ver[1]>7) {
      $options = elgg_list_entities(array(
      	'type' => 'object',
      	'subtype' => 'videoconsultation',
      	'limit' => 10,
      	'full_view' => false,
    ...
    if (!$options) {
    	$options = elgg_echo('videoconsultation:none');
    }
	...
	// Display page
  	if ($ver[1]>7) echo elgg_view_page(elgg_echo('videoconsultation:rooms'),$body, 'default', array( 'sidebar' => "" ));
    else page_draw(elgg_echo('videoconsultation:rooms'),$body);
