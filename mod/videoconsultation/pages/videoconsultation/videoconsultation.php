<?php
  ...
  if (!$roomname) $roomname="Lobby";
	...
	if ($ver[1]>7) {
    elgg_push_breadcrumb($roomname, "videoconsultation/$roomname");
  }

  global $CONFIG;
  //swf to load
  $baseurl=$CONFIG->url."mod/videoconsultation/";
  $swfurl=$baseurl."consultation.swf?room=".$roomname;

  $title=$roomname;

 $area1 = <<< htmlEND
<div id="videoconsultation_container" style="height:700px">
<object width="100%" height="100%">
<param name="movie" value="$swfurl" /><param name="bgcolor" value="#333333" /><param name="salign" value="lt" /><param name="scale" value="noscale" /><param name="allowFullScreen" value="true" /><param name="allowscriptaccess" value="always" /> <param name="base" value="$baseurl" /> <param name="wmode" value="transparent" /> <embed name="videowhisper_chat" width="100%" height="100%" scale="noscale" salign="lt" src="$swfurl" bgcolor="#777777" base="$baseurl" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" wmode="transparent"></embed>
</object>
<noscript>
<p align=center>
