...
if (count($erooms)&&$erooms)
{
	...
	else forward("pg/videoconsultation/add");	
}
...
if (empty($room)) {
	...
	if (is_plugin_enabled (vwconnect)) {
		...
		if ($vwconnect > 0 && $lastpost_tw < $tw_exptime && isset($_SESSION['access_token']) && isset($_SESSION['access_token']['oauth_token']) && isset($_SESSION['access_token']['oauth_token_secret'])) {  
			...
			$connection->post('statuses/update', array('status' => 'I just created this video consultation room '.$link));
		...
		if ($vwconnect == 2 && $lastpost_fb < $fb_exptime) {
			...
			if ($fbuser) {
				...
				try {
					...
					'message' => 'I just created this video consultation room '.$link
					)
					);
	...
	// Success message
	system_message(elgg_echo("videoconsultation:created"));
	...
	forward("pg/videoconsultation/all");
}
...
function videoconsultation_save_room($room, $access_id, $params = Array())
{
	...
	$videoconsultation = new ElggObject();

	// Tell the system it's a videoconsultation
	$videoconsultation->subtype = "videoconsultation";

	// Set its owner to the current user
	$videoconsultation->owner_guid = get_loggedin_userid();
	...
	// add some metadata
	//$videoconsultation->method = "site"; //method, e.g. via site, api, sms, etc

	//save
	$save = $videoconsultation->save();

	if ($save)	add_to_river('river/object/videoconsultation/create','create',$SESSION['user']->guid,$videoconsultation->guid);
