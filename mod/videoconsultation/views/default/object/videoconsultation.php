...
$sqllastaccesku = "SELECT ".$CONFIG->dbprefix."objects_entity.description, ".
       ...
       "ON (".$CONFIG->dbprefix."objects_entity.guid = ".$CONFIG->dbprefix."entities.guid) ".
			 ...
			 "LEFT(".$CONFIG->dbprefix."objects_entity.description,1) = '3' ORDER BY guid DESC LIMIT 1;";
...
$sqlliveusercount = "SELECT ".$CONFIG->dbprefix."objects_entity.description, ".
       ...
       "ON (".$CONFIG->dbprefix."objects_entity.guid = ".$CONFIG->dbprefix."entities.guid) ".
			 "WHERE ".$CONFIG->dbprefix."entity_subtypes.subtype = 'videowhisper' AND ".
			 $CONFIG->dbprefix."objects_entity.title = '".$object->title."' AND ".
			 "LEFT(".$CONFIG->dbprefix."objects_entity.description,3) = '3:1' ORDER BY guid DESC;";
	$count = 0;
	$ztime = time();
	$exptime=$ztime-30;
	$users = "";
    ...
    $owner_link = elgg_view('output/url', array(
    	'href' => "videoconsultation/owner/$owner->username",
    	'text' => $owner->name,
    	'is_trusted' => true,
    ));
...
?>
<div class="videoconsultation-singlepage">
	<div class="videoconsultation-room">

	    <!-- the actual shout -->
		<div class="room_body">

	    <div class="videoconsultation_icon">
	    <?php
		        echo elgg_view("profile/icon",array('entity' => $vars['entity']->getOwnerEntity(), 'size' => 'small'));
	    ?>
	    </div>

			<div class="videoconsultation_options">
	    <div class="clearfloat"></div>
	    		<?php

			// if the user looking at videoconsultation post can edit, show the delete link
			if ($vars['entity']->canEdit()) {


					    echo "<div class='delete' style='float:right;'>" . elgg_view("output/confirmlink",array(
															'href' => $vars['url'] . "action/videoconsultation/delete?videoconsultationroom=" . $vars['entity']->getGUID(),
		...
		<?php
		    echo sprintf(elgg_echo('videoconsultation:river:created'),"<b>{$user_name}</b>");
        echo " $link <BR>";
		    $descx = explode("^", $vars['entity']->description);
        $desc = preg_replace('/\@([A-Za-z0-9\_\.\-]*)/i','@<a href="' . $vars['url'] . 'videoconsultation/$1">$1</a>',$descx[0]);
