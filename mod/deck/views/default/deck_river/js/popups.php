...
elgg.deck_river.popups = function() {
	...
	// tabs popups
	$(document).on('click', '.deck-popup .elgg-tabs a, #group_activity_module .elgg-tabs a', function() {
	...
	// user info popup
	$(document).on('click', '.elgg-user-info-popup', function() {
		elgg.deck_river.userPopup($(this).attr('title'));
		return false;
	}).liveDraggable();

	// group info popup
	$(document).on('click', '.group-info-popup', function() {
	...
	// hashtag info popup
	$(document).on('click', '.hashtag-info-popup', function() {
	...
	$('.twitter-user-info-popup').on('click', function() {
		elgg.deck_river.twitterUserPopup($(this).attr('title'));
	}).liveDraggable();

	// Facebook user info popup
	$('.facebook-user-info-popup').on('click', function() {
	...
	$('.facebook-page-info-popup').on('click', function() {
		var columnRiver = $(this).closest('.column-river'),
			token = columnRiver.length ? elgg.deck_river.getColumnSettings($(this).closest('.column-river')).token : null;
	...
	$('.media-video-popup').on('click', function() {
	...
	$('.media-image-popup').on('click', function() {
		var $this = $(this),
			type = $this.data('type');
	...
	// share account
	$('.share-account').on('click', function() {
	...
	if (!$('#'+popupID).length) {
		...
		var popup = $('#'+popupID).draggable({
			...
			create: function(e, ui) {
				...
				$('#'+popupID).css('z-index', 9999999);//must be more than anything else
			}
		});
		popup.click(function() {
			$('.deck-popup').css('z-index', '-=1');
			$(this).closest('.deck-popup').css('z-index', 9999999);
		});
