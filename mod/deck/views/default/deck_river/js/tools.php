...
elgg.deck_river.getColumnSettings = function(TheColumn) {
	if (TheColumn.attr('id')) {
		var tab = TheColumn.closest('#deck-river-lists').attr('data-tab'),
...
var DataEntities = DataEntities || {elgg: [], twitter: [], facebook: [], tumblr: [], linkedin: []};
...
elgg.deck_river.storeEntity = function(entity, network) {
	...
	if (network == 'elgg') {
		...
		if (!$.grep(DataEntities.tumblr, function(e){ return e.blog_name === entity.blog_name; }).length) DataEntities.tumblr.push(entity);
	} else if (network == 'linkedin') {
		if (!$.grep(DataEntities.linkedin, function(e){ return e.id === entity.id; }).length) DataEntities.linkedin.push(entity);
	}
};
...
$('.elgg-input-dragbox > div').sortable({
		...
		connectWith: $('.elgg-input-dragbox > div'),
		helper: 'clone',
		//revert: 1,
		...
		accept:    $('.elgg-input-dragbox span'),
		activeClass: 'ui-state-highlight',
		hoverClass:  'ui-state-active',
		drop: function(e, ui) {
			//console.log(e);
			$('.elgg-input-dragbox *').removeClass('ui-start');
			//ui.draggable.removeClass('ui-start');
			if(ui.draggable.find('input').attr('name') == 'accounts[]'){
				ui.draggable.prependTo($(this)).find('input').attr('name', '_accounts[]');
			} else {
				ui.draggable.appendTo($(this)).find('input').attr('name', 'accounts[]');
...
$('.deck-scheduler-button').on('click',function(){
...
$(document).on("click", ".attached > .deck-attachment-button", function (e){
	e.preventDefault();
    $('.deck-attachment-button').replaceWith($('.deck-attachment-button').clone( true ));
    $('.deck-attachment-button').parent().removeClass('attached');
});
...
$('#post-input-box').on('keyup', function(e){
	...
	if (url.length) {
		$('.deck-post-preview').trigger('preview', url);
	}
});

var urlLoaded = false;

$('.deck-post-preview').on('preview', function(e, url){
	var $elem = $(this);
	...
	if (url) {
		elgg.get('https://iframely.com/iframely', {
			...
			success: function(data) {
				...
				$('.deck-post-preview').show();
				$('#post-input-box').css('height', '100px');
				if(data.meta.title){
					$('.deck-post-preview .deck-post-preview-title').val(data.meta.title);
				}
				
				if(data.meta.description){
					$('.deck-post-preview .deck-post-preview-description').html(data.meta.description);
				}
				...
				data.links.forEach(function(link) {
					...
					if($.inArray('thumbnail', link.rel)>=0){
						$('.deck-post-preview .deck-post-preview-icon-img').attr('src', link.href);
						$('.deck-post-preview .deck-post-preview-icon').val(link.href);
					}
				});
				
				$('.deck-post-preview .deck-post-url').val(url);
			//	var icon = data.
