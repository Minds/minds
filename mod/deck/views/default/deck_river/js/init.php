...
elgg.deck_river.ColumnSettings = function(TheColumn) {
	...
	var tabGUID = $('.elgg-river-layout:not(.hidden) #deck-river-lists').attr('data-tab');
	...
	elgg.post('ajax/view/deck_river/ajax_view/column_settings', {
		...
		success: function(response) {
			...
			$network = $cs.find('.elgg-tabs > li.elgg-state-selected').find('a').attr('class');
			...
			$cs.find('.elgg-tabs.networks a').click(function() {
				...
				$cs.find('.tab').addClass('hidden');
				$cs.find('.tab.'+$etn).removeClass('hidden');
			...
			$('.elgg-foot .elgg-button').click(function() {
				...
				elgg.action('deck_river/column/settings', {
					data: $drfcs.serialize() + '&network=' + $network,
					dataType: 'json',
					success: function(json) {
...
//p.console.log(token);
	if (token == false) {
		...
		// add new network account
		p.$('.elgg-form-deck-river-post .deck-river-accounts .selected').prepend('<span>'+token.network_box+'</span>');
...
elgg.deck_river.MoveColumn = function(event, ui) {
	...
	elgg.action('deck_river/column/move', {
		data: {
			tab: ui.item.closest('.elgg-river-layout:not(.hidden) #deck-river-lists').attr('data-tab'),
...
elgg.deck_river.SetColumnsWidth = function() {
	...
	$erl.find('.column-river, .column-placeholder').width(ListWidth - 12);
