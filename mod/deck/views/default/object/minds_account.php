...
if ($view === 'in_network_box') {
	...
	$node_url = $account->node != 'local' ? $account->node : elgg_get_site_url();

	$avatar = elgg_view('output/img', array(
		'src' => "$node_url/icon/$account->id/small", // $account->avatar,
		'alt' => $account->name,
		'title'=> "$account->network - @$account->name",
		'class' => 'avatar tooltip s',
	));

	$info = '<div class="elgg-river-summary"><span class="twitter-user-info-popup info-popup" title="' . $account->name . '">' . $account->name . '</span>';
	$info .= '<br/><span class="elgg-river-timestamp">';
	$info .= elgg_view('output/url', array(
		'href' => 'http://twitter.com/' . $account->name,
		'text' => 'http://twitter.com/' . $account->name,
		'target' => '_blank',
		'rel' => 'nofollow'
	));
	$info .= '</span></div>';
