...
if ($view === 'in_network_box') {
	...
	$avatar = elgg_view('output/img', array(
		'src' => 'https://twitter.com/api/users/profile_image/' . $account->screen_name . '?size=mini', // $account->avatar,
	...
	$info .= elgg_view('output/url', array(
		...
		'text' => 'https://twitter.com/' . $account->screen_name,
		'target' => '_blank',
		'rel' => 'nofollow'
	));
	$info .= '</span></div>';
...
HTML;
	...
	$avatar = elgg_view('output/img', array(
		'src' => 'https://twitter.com/api/users/profile_image/' . $account->screen_name . '?size=normal', // $account->avatar,
		'alt' => $account->screen_name,
		'class' => 'float',
	));
