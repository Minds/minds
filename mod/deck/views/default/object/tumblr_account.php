 ...
 *
 * View for tumblr_account object
 *
...
if ($view === 'in_network_box') {
	...
	$avatar = elgg_view('output/img', array(
		'src' => "https://api.tumblr.com/v2/blog/{$account->screen_name}.tumblr.com/avatar/24",
		'alt' => $account->screen_name,
		'title'=> "$account->network - @$account->screen_name",
		'class' => 'float avatar tooltip s',
	));

	$info = '<div class="elgg-river-summary"><span class="tumblr-user-info-popup info-popup" title="' . $account->screen_name . '">' . $account->screen_name . '</span>';
	$info .= '<br/><span class="elgg-river-timestamp">';
	$info .= elgg_view('output/url', array(
		'href' => "https://{$account->screen_name}.tumblr.com/",
		'text' => "https://{$account->screen_name}.tumblr.com/",
		'target' => '_blank',
		'rel' => 'nofollow'
	));
	$info .= '</span></div>';
...
HTML;
	...
	$avatar = elgg_view('output/img', array(
		'src' => "https://api.tumblr.com/v2/blog/{$account->screen_name}.tumblr.com/avatar/48",
		'alt' => $account->screen_name,
		'class' => 'float',
	));
	...
	$author_text = elgg_echo('deck_river:account:createdby', array('Tumblr', elgg_get_site_entity()->name, $owner_link));
	...
	<div class="span8">
		<div class="elgg-image-block clearfix">
			<div class="elgg-image">
				<span title="{$account->screen_name}" class="tumblr-user-info-popup info-popup">$avatar</span>
			</div>
			<div class="elgg-body">
				...
				<h3><span class="tumblr-user-info-popup info-popup" title="{$account->screen_name}">{$account->screen_name}</span></h3>
				$link
